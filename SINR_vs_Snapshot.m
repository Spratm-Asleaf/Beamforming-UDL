...
%% Output SINR Against Snapshots

snr = 10^(SNR/10);
Pv = Ps(1)/snr;

% True Cov of received data
Rx0 = a0(N, Theta)*diag(Ps)*a0(N, Theta)' + Pv*eye(N);
% True Cov of IPN
Ripn0 = Rx0 - Ps(1)*a0(N, Theta(1))*a0(N, Theta(1))';

% Data Storage
SnapshotLength  = length(Snapshot);

SINR_Capon0     = zeros(MonteCarlo, SnapshotLength);
SINR_Capon      = zeros(MonteCarlo, SnapshotLength);
SINR_Capon_DL   = zeros(MonteCarlo, SnapshotLength);
SINR_IPN0       = zeros(MonteCarlo, SnapshotLength);
SINR_IPN        = zeros(MonteCarlo, SnapshotLength);
SINR_IPN_DL     = zeros(MonteCarlo, SnapshotLength);
SINR_IPN_UDL    = zeros(MonteCarlo, SnapshotLength);
SINR_IPN_MatEnt = zeros(MonteCarlo, SnapshotLength);

for i = 1:SnapshotLength
    L = Snapshot(i);
